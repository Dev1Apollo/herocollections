...
if ($_POST['action'] == 'ListUser') {
    ...
    $filterrt = "SELECT count(*) as totalcount,application.fos_completed_status FROM `application` where   application.stateid = '" . $_REQUEST['stateid'] . "' and application.agencyid = '" . $_REQUEST['agencyid'] . "' and application.am_accaptance = 1 group by application.fos_completed_status";
                            ...
                            <table class="table table-bordered dt-responsive" width="100%">
                                ...
                                while ($Dispositionwise = mysqli_fetch_array($resultfilterrt)) {
                                    ...
                                    <tr style="background-color: #DBEEF3; color: #000 !important ;">
                                        ...
                                        <td class="text-center text-light">
                                            <a onclick="checkb4submit('<?php echo $_REQUEST['stateid']; ?>', 'Disposition Summary', '<?php echo $_REQUEST['agencyid']; ?>','<?php echo $Dispositionwise['fos_completed_status']; ?>','<?php echo $_REQUEST['locationid']; ?>');"><?php echo $Dispositionwise['totalcount']; ?></a>
                                        </td>
                                    </tr>
                                    <?php
                                }
...
<script type="text/javascript">
    ...
    function checkb4submit(stateid, counttype, agencyid,fos_completed_status,locationid)
    {
        var strURL = "cmindexcountReportDownload.php?stateid=" + stateid + "&counttype=" + counttype + "&agencyid=" + agencyid + "&fos_completed_status=" + fos_completed_status + "&locationid=" + locationid;
